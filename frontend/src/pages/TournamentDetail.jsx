...
function TournamentDetail() {
    ...
    try {
      ...
      if (resp.ok) {
        ...
        if (data.autoGeneratedNextRound) {
          setNotification({ message: 'Výsledek uložen a další kolo play-off automaticky vygenerováno!', type: 'success' });
        } else {
          setNotification({ message: 'Výsledek byl uložen', type: 'success' });
        }
        ...
        setNotification({ message: err.error || 'Chyba při ukládání výsledku', type: 'error' });
      }
    } catch (e) {
      console.error(e);
      setNotification({ message: 'Chyba při spojení se serverem', type: 'error' });
  ...
  const resetMatchResult = async (matchId) => {
    try {
      ...
      if (resp.ok) {
        await fetchMatches();
        await fetchStandings();
        setNotification({ message: 'Výsledek byl resetován', type: 'success' });
        setTimeout(()=>setNotification({ message:'', type:'' }), 3000);
        setEditingMatchId(null);
        setEditingScore('');
      } else {
        const err = await resp.json();
        setNotification({ message: err.error || 'Chyba při resetování výsledku', type: 'error' });
      }
    } catch (e) {
      console.error(e);
      setNotification({ message: 'Chyba při spojení se serverem', type: 'error' });
  ...
  const handleMatchStateUpdate = async (matchId, payload, successMessage) => {
    try {
      ...
      if (resp.ok) {
        await fetchMatches();
        setNotification({ message: successMessage || 'Stav zápasu aktualizován', type: 'success' });
        setTimeout(()=>setNotification({ message:'', type:'' }), 2500);
      } else {
        const err = await resp.json();
        setNotification({ message: err.error || 'Chyba při změně stavu zápasu', type: 'error' });
      }
    } catch (e) {
      console.error(e);
      setNotification({ message: 'Chyba při spojení se serverem', type: 'error' });
  ...
  const getPlayoffRoundName = (matchNumber, totalMatchesInRound) => {
    ...
    if (totalMatchesInRound === 1) return 'Finále';
    if (totalMatchesInRound === 2) return 'Semifinále';
    if (totalMatchesInRound === 4) return 'Čtvrtfinále';
    if (totalMatchesInRound === 8) return 'Osmifinále';
    if (totalMatchesInRound === 16) return 'Šestnáctifinále';
  ...
  const renderStandingsTable = (rows) => {
    if (!rows || rows.length === 0) return (
      <div className="empty-players"><p>Zatím nejsou k dispozici žádná data pro tabulku</p></div>
    );
    return (
        ...
        {activeTab === 'prehled' && (
          ...
          <div className="info-section">
            ...
            <div className="info-item">
              <div className="info-icon">🧾</div>
              <div className="info-details">
                <div className="info-label">Počet stolů</div>
        ...
        {activeTab === 'tabulka' && (
          <div className="info-section">
            ...
            {standings && standings.type === 'pavouk' && (
              <div className="empty-players"><p>Pořadí není dostupné pro vyřazovací systém.</p></div>
            )}
